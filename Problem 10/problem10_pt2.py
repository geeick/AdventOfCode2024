...
def trailHeadScore(map, x, y, pathCount, seen):
    prev = int(map[x][y])
    ...
    for path in paths:
        ...
        if prev == 8 and int(map[x][y]) == 9:
            pathCount+=1
            if (x,y) not in seen:
                seen.add((x, y))
        else:
            pathCount, seen = trailHeadScore(mapCopy, x, y, pathCount, seen)
    return pathCount, seen
...
def addSum(map, trailheads):
    ...
    for trailhead in trailheads:
        seen = set()
        x, y = trailhead
        pathCount, seen = trailHeadScore(map, x, y, 0, seen)
        total += pathCount
